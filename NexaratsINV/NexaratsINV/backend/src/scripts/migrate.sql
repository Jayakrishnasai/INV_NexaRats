...
BEGIN
  ...
  -- ── Update customer aggregates (FIX B5: moved into atomic transaction) ─────
  IF p_customer_id IS NOT NULL THEN
    UPDATE customers
    SET
      total_paid = total_paid + p_paid_amount,
      pending = pending + (p_total - p_paid_amount),
      total_invoices = total_invoices + 1,
      last_transaction = p_date,
      status = CASE
        WHEN (pending + (p_total - p_paid_amount)) <= 0 THEN 'Paid'
