...
> Production-ready WhatsApp messaging service built on `whatsapp-web.js`, integrated into the NexaratsINV MongoDB backend.
...
│  ┌──────────────────────────────────────────────────────┐   │
│  │                    MongoDB Atlas                      │   │
│  │  • WhatsAppSession (status, metadata)                 │   │
│  │  • MessageLog (queue, sent, failed, audit trail)      │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
| **Models** | `models/WhatsAppSession.js` | Mongoose schemas for session metadata + message audit log |
...
| `mongoose` | ^9.x | Already installed — MongoDB ODM |
...
1. **LocalAuth strategy** — `whatsapp-web.js` stores the Chromium session profile in `mongodb-service/.wwebjs_auth/`
...
5. Session metadata (status, phone number, push name, timestamps) is also stored in MongoDB (`whatsAppSession` collection) for monitoring
...
├── models/
│   └── WhatsAppSession.js        # Mongoose schemas
├── routes/
...
```
Server Start → MongoDB Connect → WhatsApp Initialize
                                       │
                              ┌────────▼────────┐
                              │  Generate QR     │
                              │  (or load saved  │
